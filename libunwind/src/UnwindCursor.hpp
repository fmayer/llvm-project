...
public:
  ...
  virtual int step(bool clean = false) {
...
private:
  ...
  int stepWithDwarfFDE(bool clean) {
    return DwarfInstructions<A, R>::stepWithDwarf(
        _addressSpace, (pint_t)this->getReg(UNW_REG_IP),
        (pint_t)_info.unwind_info, _registers, _isSignalFrame, clean);
  }
#endif
  ...
  int stepWithCompactEncoding(bool clean = false) {
#if defined(_LIBUNWIND_SUPPORT_DWARF_UNWIND)
    if ( compactSaysUseDwarf() )
      return stepWithDwarfFDE(clean);
#endif
    R dummy;
...
template <typename A, typename R> int UnwindCursor<A, R>::step(bool clean) {
